...
let wishes=JSON.parse(localStorage.getItem("diamondEmpireWishes")||"[]");
...
function saveCart(){localStorage.setItem("diamondEmpireCart",JSON.stringify(cart));renderCart()}
function saveWishes(){localStorage.setItem("diamondEmpireWishes",JSON.stringify(wishes))}
...
// Boutique reveal + subtle pointer ambience
const revealObserver=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add("revealed");revealObserver.unobserve(entry.target)}})},{threshold:.12,rootMargin:"0px 0px -35px 0px"});
document.querySelectorAll("[data-reveal]").forEach((el,i)=>{el.style.transitionDelay=`${Math.min(i*45,180)}ms`;revealObserver.observe(el)});
const cursorLight=document.getElementById("cursorLight");
window.addEventListener("pointermove",e=>{if(window.innerWidth>900){cursorLight.style.left=e.clientX+"px";cursorLight.style.top=e.clientY+"px"}},{passive:true});
...
const navObserver=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting){const id=entry.target.id;navLinks.forEach(a=>a.classList.toggle("active",a.getAttribute("href")===`#${id}`))}})},{rootMargin:"-45% 0px -45% 0px",threshold:0});
