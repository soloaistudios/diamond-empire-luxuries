...
function closeCartHard(event){
  if(event) event.preventDefault();
  closeCart();
}

const closeCartButton=document.getElementById("closeCart");
closeCartButton.addEventListener("click",closeCartHard);
closeCartButton.addEventListener("pointerup",closeCartHard);
closeCartButton.addEventListener("touchend",closeCartHard,{passive:false});

pageOverlay.addEventListener("click",closeCartHard);
pageOverlay.addEventListener("pointerup",closeCartHard);
pageOverlay.addEventListener("touchend",closeCartHard,{passive:false});

document.addEventListener("click",event=>{
  const closeTarget=event.target.closest("[data-close-cart]");
  if(closeTarget) closeCartHard(event);
});
